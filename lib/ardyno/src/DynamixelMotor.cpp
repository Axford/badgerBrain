...
float DynamixelDevice::voltage() {
	uint8_t result;
	DynamixelStatus status = read(42, result);
	if (status!=DYN_STATUS_OK) {
		return -1;
	}
	return result / 10.0f;
...
// read present load
// converted to a signed % value...  negative values are CCW, positive are CW
float DynamixelDevice::torque() {
	uint16_t result;
	float t;
	DynamixelStatus status = read(40, result);
	//Serial.print("t:");
	//Serial.println(result);
	if (status!=DYN_STATUS_OK || result > 2047) {
		return 0;
	}
	t = result > 1023 ? (result-1023)/1023.0f : -result/1023.0f;
	return t;
}
...
DynamixelStatus DynamixelMotor::enableTorque(bool aTorque)
{
