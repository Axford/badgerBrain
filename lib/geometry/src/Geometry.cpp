...
float Point::AngleTo(Point &obj) {
  float denominator = sqrt(Magnitude() * obj.Magnitude());
  if (denominator == 0) return PI/2;

  float theta = DotProduct(obj) / denominator;

  return acos(max(min(theta,1.0f),-1.0f));
}

void Point::RotateZ(float ang) {
  float x2 = cos(ang)* (*this)(0) - sin(ang) * (*this)(1);
  float y2 = sin(ang)* (*this)(0) + cos(ang) * (*this)(1);
  (*this)(0) = x2;
  (*this)(1) = y2;
}

boolean Point::EqualTo(Point &obj) {
  return (*this)(0) == obj(0) && (*this)(1) == obj(1) && (*this)(2) == obj(2);
}
